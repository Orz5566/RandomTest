                ...
                if (!this.bチップがある.LP && !this.bチップがある.LBD)
                {
                    ...
                    foreach (CChip chip in this.listChip)
                    {
                        if (chip.nチャンネル番号 == 0x13)
                        {
                            ...
                            // ズレた配置への配慮も含めて、試しに148ms（BPM202.7の8分）を境にしてみた
                            if (!bLBDに変更した && chip.n発声時刻ms - n前のBDチップ発声時刻 < 148)
                            {
                                chip.nチャンネル番号 = 0x1c;
                            }
                            // 前の発声位置とフラグを更新
                            n前のBDチップ発声時刻 = chip.n発声時刻ms;
            ...
            if (eDkdkType == Eタイプ.B)
            {
                ...
                foreach (CChip chip in this.listChip)
                {
                    ...
                    if ((n元チャンネル番号 == 0x13) || (n元チャンネル番号 == 0x1c))
                    {
                        nバスチップ番号++;
                        // BDとLBD同時押しがあった場合
                        if (((n前の元チャンネル番号 == 0x13) && (chip.n発声時刻ms == n前のBD発声時刻)) || ((n前の元チャンネル番号 == 0x1c) && (chip.n発声時刻ms == n前のLBD発声時刻)))
                        {
                        ...
                        {
                            // 前に元BDがあって且つ発声時刻差が300ms（BPM200の4分）以内だったらドコドコとみなし、
                            // BDとLBDを入れ替わる（ズレ配慮で296msにしてみた）
                            if ((n前の元チャンネル番号 == 0x13) && ((chip.n発声時刻ms - n前のBD発声時刻) <= 296))
                            {
                                if (!bドコドコ中)
                                {
                                    if (!b直前はバス同時押し)
                                ...
                                chip.nチャンネル番号 = 0x13;
                                nドコドコ中前後チップの時刻差 = chip.n発声時刻ms - n前のBD発声時刻;
                            }
                            else
                            {
                                // ドコドコ状態を解除
                        ...
                        {
                            // 前に元LBDがあって且つ発声位置が296ms（BPM202.7の4分）以内だったら
                            if ((n前の元チャンネル番号 == 0x1c) && ((chip.n発声時刻ms - n前のLBD発声時刻) <= 296))
                            {
                                // ここらへんはドコドコ中の判断とチップの移行を行っている
                                // 変数名を日本語にしたにも関わらず、具体的に何をしているかはもう分からなくなった（汗
                                if (!bドコドコ中)
                                {
                                    if ((((chip.n発声時刻ms - n前のBD発声時刻) - nドコドコ中前後チップの時刻差) < 50) || (nドコドコ中前後チップの時刻差 == -1))
                                    {
                                        n交換待ちのバスチップ番号配列[i++] = nバスチップ番号 - 1;
                                        bドコドコ中 = true;
                                        chip.nチャンネル番号 = 0x1c;
                                ...
                                else if (((chip.n発声時刻ms - n前のLBD発声時刻) - nドコドコ中前後チップの時刻差) >= 0x10)
                                {
                                    bドコドコ中 = false;
                                }
                                else
                                ...
                                nドコドコ中前後チップの時刻差 = chip.n発声時刻ms - n前のLBD発声時刻;
                            }
                            else
                            {
                                // ドコドコ状態を解除
                        ...
                        if (n元チャンネル番号 == 0x13)
                        {
                            n前のBD発声時刻 = chip.n発声時刻ms;
                        }
                        else
                        {
                            n前のLBD発声時刻 = chip.n発声時刻ms;
                        }
                    }
                }
                // 変数を初期化
